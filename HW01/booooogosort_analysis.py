...
def sameElement(new_list,array):
    return collections.Counter(new_list) == collections.Counter(array)
    ...
    def run(self):
        ...
        while True:
            if self.output.empty():
                new_list = [random.choice(self.array) for _ in self.array]
                self.count += self.length   # not just one cause we gotta check all items for sorted
                if is_sorted(new_list) and sameElement(new_list,self.array):
